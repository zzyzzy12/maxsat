...
int degree[NB_V];
bool change=false;
void branch(int &n,int &m,int n0,int m0,int *X,int &maxNum,int *ans,set<int> *C,set<int> *C0,int *TP,node* H,int Upbound,double current,double rest){
	...
	if (k){
		...
		X[k]=0;
//-------------------------------------------
        if(rest>=0.0001) rest=rest/2;
//-------------------------------------------
		branch(n,m,n0,m0,X,maxNum,ans,C,C0,TP,H,Upbound,current,rest);
//-------------------------------------------
		if(rest>=0.0001){
		current=current+rest;
		printf("processing in percent %lf %%\r",current);
		}
//-------------------------------------------
		X[k]=1;
		branch(n,m,n0,m0,X,maxNum,ans,C,C0,TP,H,Upbound,current,rest);
...
int main(int argc,char **arg){
	...
	branch(n,m,n,m,X,maxNum,ans,C,C0,TP,H,m,current,rest);
	puts("---------------------------------------------");
	printf("%d\n",maxNum);
	for (int i=1;i<=n0;i++){
        if(ans[i]) printf("%d ", i);
